...
public class TraceSchedule {
  ...
  java.util.Dictionary<Temp.Label,Tree.StmList> table = new java.util.Hashtable<Temp.Label,Tree.StmList>();
    ...
    for(;;) {
      ...
      if (s instanceof Tree.JUMP) {
        ...
        if (f != null) {
          ...
          last.tail.head
            = new Tree.CJUMP(j.relop, j.left, j.right, j.iftrue, ff);
          last.tail.tail
            = new Tree.StmList(new Tree.LABEL(ff),
